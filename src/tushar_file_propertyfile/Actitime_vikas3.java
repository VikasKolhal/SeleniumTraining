...
public class Actitime_vikas3 {
	...
	//	driver.findElement(By.cssSelector("//table[@class='x-btn-wrap x-btn at-dropdown-list-btn dateButton descendantEventsEmitter ancestorEventsListener  x-btn-over ']//em/button")).click();
		...
		List<String> actualTaskName=new ArrayList<String>();
		  ...
		  if(actualTaskName.contains("QA-Testing-500")) {
		...
		List<String> taskNamesAfterDelation=new ArrayList<String>();
		for(int i=0;i<taskList2.size();i++) {
			taskNamesAfterDelation.add(taskList2.get(i).getText());
			  
		  }
		System.out.println("Actuatal Task list: "+taskNamesAfterDelation);
		  
		  if(taskNamesAfterDelation.contains("QA-Testing-500")) {
